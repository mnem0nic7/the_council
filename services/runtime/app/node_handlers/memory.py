...
class MemoryNodeHandler:
    ...
    async def execute(self, node: WorkflowNode, ctx: ExecutionContext) -> NodeResult:
        ...
        result = await self._executor._run_memory_node(ctx.run_id, node, context)
        return NodeResult(payload=result.payload, route=result.route)


def make_handler(executor: MissionExecutor) -> MemoryNodeHandler:
    handler = MemoryNodeHandler(executor)
